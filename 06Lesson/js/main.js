...
function createHTML(film) {
    ...
    <div class="card" id="card_${film[x].show.id}">
        ...
        if (film[x].show.image !== null) {
            document.getElementById(`image_${film[x].show.id}`).src = film[x].show.image.medium
        }
        //insert discriptions
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            if (elem in film[x].show) {
                ...
                if (elem === 'genres') {
                    ...
                    li.innerHTML = `${elem[0].toUpperCase() + elem.slice(1)}: ${film[x].show[elem]}`
                    document.getElementById(film[x].show.id).append(li)
                }
            }
        }
        //delete item
        document.getElementById(`clouse_${film[x].show.id}`).onclick = function (e) {
